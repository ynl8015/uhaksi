...
function parseCategory(s: string | null): CommunityCategory {
  if (s === 'STUDY_TIP' || s === 'STUDY_PROOF' || s === 'QA') return s
  return 'QA'
}
...
function WriteInner() {
  ...
  const initialCat = parseCategory(searchParams.get('category'))

  const [tab, setTab] = useState<CommunityCategory>(initialCat)
  ...
  if (status === 'unauthenticated' || !session?.user || !canAccessStudentCommunity(session.user)) {
    return (
      <main style={{ minHeight: 'calc(100vh - 64px)', background: shell.pageBg, padding: '28px 20px' }}>
